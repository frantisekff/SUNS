...
def create4Histograms(data_ax0,data_ax1,data_ax2,data_ax3, title_ax0,title_ax1,title_ax2,title_ax3 ):
    ...
    ax0.set_ylabel('Pravdepodobnosť')
    ax1.set_ylabel('Pravdepodobnosť')
    ax2.set_ylabel('Pravdepodobnosť')
    ax3.set_ylabel('Pravdepodobnosť')


    ax0.plot(data_ax0.keys(), data_ax0.values(), 'bo', data_ax0.keys(), data_ax0.values(), 'k')

    ax1.plot(data_ax1.keys(), data_ax1.values(), 'bo', data_ax1.keys(), data_ax1.values(), 'k')

    ax2.plot(data_ax2.keys(), data_ax2.values(), 'bo', data_ax2.keys(), data_ax2.values(), 'k')

    ax3.plot(data_ax3.keys(), data_ax3.values(), 'bo', data_ax3.keys(), data_ax3.values(), 'k')
...
num_nodes = 1024
batch_field = [128, 128*5, 128*7, 128*8]
batch_size = 0
...
#beta = [ 0.01, 0.01, 0.1, 1,100,1000, 5000, 10000]
beta =0.01
counter = 0
...
with graph.as_default():
    ...
    tf_train_dataset = tf.placeholder(tf.float32, shape=(None, image_size * image_size*3))
    tf_train_labels = tf.placeholder(tf.float32, shape=(None, num_nodes))
...
def reshape(batch_size,x):
    return tf.reshape(x, tf.stack([batch_size, -1]))

num_steps = 1000
test_accuracy = {}
listOfTrainAccuracy = []

for i in range(len(batch_field)):
    batch_size = batch_field[i]
    dictTrainAccuracy = {}
    ...
    with tf.Session(graph=graph) as session:
        ...
        print("Initialized")
        print("pre Beta: " + format(batch_size))
        for step in range(num_steps):
            ...
            if (step % 200 == 0):
                tempTrainAcc = round(accuracy(predictions, batch_labels), 2)
                dictTrainAccuracy[step] = tempTrainAcc
                print("Minibatch loss at step {}: {}".format(step, l))
                print("Minibatch accuracy: {:.1f}".format(tempTrainAcc))
                print("Validation accuracy: {:.1f}".format(accuracy(valid_prediction.eval(), valid_labels)))
        listOfTrainAccuracy.append(dictTrainAccuracy)
        temp_test = round(accuracy(test_prediction.eval(), test_labels), 2)
        test_accuracy[batch_size] = temp_test
...
create4Histograms(OrderedDict(sorted(listOfTrainAccuracy[0].items(), key=lambda t: t[0])),OrderedDict(sorted(listOfTrainAccuracy[1].items(), key=lambda t: t[0])), OrderedDict(sorted(listOfTrainAccuracy[2].items(), key=lambda t: t[0])), OrderedDict(sorted(listOfTrainAccuracy[3].items(), key=lambda t: t[0])), 10, 20, 30, 50 )
